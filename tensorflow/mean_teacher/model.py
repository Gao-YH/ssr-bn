...
def tower(inputs,
    ...
    with tf.name_scope(name, "tower"):
        ...
        slim.arg_scope(training_mode_funcs, **training_args):
            ...
            net = wn.conv2d(net, 64, scope="conv_1_1")
            net = wn.conv2d(net, 64, scope="conv_1_2")
            net = wn.conv2d(net, 64, scope="conv_1_3")
            net = slim.max_pool2d(net, [2, 2], scope='max_pool_1')
            net = slim.dropout(net, 1 - dropout_probability, scope='dropout_probability_1')
            assert_shape(net, [None, inputsize/2, inputsize/2, 64])

            net = wn.conv2d(net, 128, scope="conv_2_1")
            net = wn.conv2d(net, 128, scope="conv_2_2")
            net = wn.conv2d(net, 128, scope="conv_2_3")
            net = slim.max_pool2d(net, [2, 2], scope='max_pool_2')
            net = slim.dropout(net, 1 - dropout_probability, scope='dropout_probability_2')
            assert_shape(net, [None, inputsize/4, inputsize/4, 128])
            ...
            assert_shape(net, [None, inputsize/8, inputsize/8, 256])

            net = wn.conv2d(net, 128, kernel_size=[1, 1], scope="conv_3_2")
            net = wn.conv2d(net, 64, kernel_size=[1, 1], scope="conv_3_3")
            net = slim.avg_pool2d(net, [inputsize/8, inputsize/8], scope='avg_pool')
            assert_shape(net, [None, 1, 1, 64])

            net = slim.flatten(net)
            assert_shape(net, [None, 64])
